...
const getUserQuizCodes = async (req, res) => {
  try {
    ...
    // Filter quizzes to ensure quizCode contains a hyphen
    const validQuizzes = quizzes.filter(quiz => quiz.quizCode.includes('-'));

    // Remove duplicate quizCode entries if necessary
    const uniqueQuizzes = [...new Map(validQuizzes.map(quiz => [quiz.quizCode, quiz])).values()];
